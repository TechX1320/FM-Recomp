...
/* Drive Init (0x0A) second-response latency. Authentic hardware takes roughly
 * 1.136M CPU cycles, but some BIOS/libcd combinations use a shorter timeout
 * and reissue Init before that response becomes visible. Keep the authentic
 * default and expose an opt-in compatibility override for per-game testing. */
static int cdrom_init_complete_delay = 1136000;

/* ReadTOC (0x1E) second-response latency. The generic runtime default follows
 * Beetle's near-second model (30,000,000 CPU cycles). Forbidden Memories'
 * observed startup window is only about 49-50 NTSC frames, and the legacy
 * pending-response model can pause the countdown behind the immediate ACK.
 * Keep the generic default and expose a narrow per-game compatibility override. */
static int cdrom_readtoc_complete_delay = 30000000;

/* Optional command-pipeline compatibility mode. Real drive work continues
 * while an earlier response waits for the guest to acknowledge it; only the
 * later response presentation is serialized. The legacy model froze the
 * entire pending-command countdown whenever irq_flag was nonzero, which can
 * push long commands such as ReadTOC beyond a game's own timeout. */
static int cdrom_pending_time_continues = 0;

/* Yu-Gi-Oh! Forbidden Memories APv2 compatibility. The publisher-logo
 * overlay performs a deliberate anti-modchip probe: seek to its calculated test position, issue
 * Play, then use Test 19h/04h and 19h/05h to look for repeated SCEx symbols.
 * A normal single-data-track image has no playable CD-DA at that data-track position, but real
 * hardware still acknowledges the Play command used by this probe. Keep this
 * behavior opt-in and game-specific so generic invalid Play requests continue
 * to return INT5. */
static int fm_apv2_stock_mode;
static int fm_apv2_probe_active;
static uint8_t fm_apv2_total;
static uint8_t fm_apv2_success;
static uint64_t fm_apv2_probe_play_acks;
static uint64_t fm_apv2_test04_count;
static uint64_t fm_apv2_test05_count;

/* Exact, signature-gated fallback matching the established Forbidden Memories
 * APv2 master-code patch. It changes only the dispatcher branch at 80168188
 * after the complete three-word signature is present in live RAM. Marking the
 * page dirty forces execution from patched RAM instead of stale static code. */
static int fm_apv2_bypass_enabled;
static int fm_apv2_bypass_applied;
static uint32_t fm_apv2_bypass_frame;
static uint32_t fm_apv2_bypass_seen_184;
static uint32_t fm_apv2_bypass_seen_188;
static uint32_t fm_apv2_bypass_seen_18c;
static uint64_t fm_apv2_bypass_last_check_frame = UINT64_MAX;
...
static void fm_apv2_try_apply_bypass(void) {
    if (!fm_apv2_bypass_enabled || fm_apv2_bypass_applied) return;
    if (fm_apv2_bypass_last_check_frame == s_frame_count) return;
    fm_apv2_bypass_last_check_frame = s_frame_count;

    uint8_t *ram = memory_get_ram_ptr();
    if (!ram) return;
    const uint32_t phys = 0x00168188u;
    fm_apv2_bypass_seen_184 = read_le32(ram + phys - 4u);
    fm_apv2_bypass_seen_188 = read_le32(ram + phys);
    fm_apv2_bypass_seen_18c = read_le32(ram + phys + 4u);
    if (fm_apv2_bypass_seen_184 != 0x2C620014u ||
        fm_apv2_bypass_seen_188 != 0x1040023Au ||
        fm_apv2_bypass_seen_18c != 0x00031080u)
        return;

    write_le32(ram + phys, 0x1000023Au);
    dirty_ram_mark_executable_range(phys, 4u);
    fm_apv2_bypass_seen_188 = 0x1000023Au;
    fm_apv2_bypass_applied = 1;
    fm_apv2_bypass_frame = (uint32_t)s_frame_count;
}
...
static void exec_command(uint8_t cmd) {
    ...
    case 0x0A: /* Init */
        ...
        pending.delay = cdrom_init_complete_delay;
        pending.phase = 1;
        break;
    ...
    case 0x03: /* Play — start CD-DA audio playback (from SetLoc, or optional
        ...
        }
        {
            ...
            if (!start_cdda_playback(requested_track)) {
                int apv2_probe_position =
                    requested_track == 0 && s_setloc_lba == 109800 &&
                    seek_min == 24 && seek_sec == 26 && seek_sect == 0 &&
                    cd_muted && iso_handle && iso_track_count(iso_handle) == 1 &&
                    !iso_track_is_audio(iso_handle, 1);
                if (fm_apv2_stock_mode && apv2_probe_position) {
                    /* APv2 deliberately probes Play at the calculated
                     * 24:26:00 data-track position. Acknowledge without
                     * starting host audio; Test 19h/04h and 19h/05h perform
                     * the actual SCEx observation. */
                    stat_reg = (stat_reg & ~(CDSTAT_SEEK | CDSTAT_READ)) |
                               CDSTAT_MOTOR | CDSTAT_PLAY;
                    fm_apv2_probe_play_acks++;
                } else {
                    response_push(stat_reg | CDSTAT_ERROR);
    ...
    case 0x1E: /* ReadTOC */
        ...
        pending.delay = cdrom_readtoc_complete_delay;
        pending.phase = 1;
        break;

    case 0x19: /* Test */
        if (fm_apv2_stock_mode && param_count >= 1 && param_fifo[0] == 0x04) {
            /* Reset SCEx counters and begin the APv2 observation window.
             * The emulator has no physical modchip injecting post-boot SCEx,
             * so the clean-console counters start and remain at zero. */
            fm_apv2_probe_active = 1;
            fm_apv2_total = 0;
            fm_apv2_success = 0;
            fm_apv2_test04_count++;
            stat_reg |= CDSTAT_MOTOR;
            response_push(stat_reg);
            set_irq(CDIRQ_ACK);
        } else if (fm_apv2_stock_mode && param_count >= 1 && param_fifo[0] == 0x05) {
            /* Return total SCEx symbols and successful decodes. Forbidden
             * Memories expects no repeated symbols during this post-boot test. */
            fm_apv2_probe_active = 0;
            fm_apv2_test05_count++;
            response_push(fm_apv2_total);
            response_push(fm_apv2_success);
            set_irq(CDIRQ_ACK);
        } else if (param_count >= 1 && param_fifo[0] == 0x20) {
...
static void process_pending(uint32_t cycles) {
    ...
    if (cdrom_pending_time_continues) {
        /* The drive operation keeps advancing even while the immediate ACK is
         * still visible. Once its deadline is reached, serialize only the
         * response presentation until the guest clears the earlier IRQ. */
        if (pending.delay > 0) {
            if ((uint32_t)pending.delay > cycles)
        ...
        /* Legacy behavior retained for the generic comparison launcher. */
        if (irq_flag != 0) return;
        pending.delay -= (int)cycles;
        if (pending.delay > 0) return;
    }
...
void cdrom_init(const char* cue_path) {
    ...
    cdrom_init_complete_delay = 1136000;
    {
        const char* e = getenv("PSX_CD_INIT_DELAY");
        if (e && e[0]) {
            long v = strtol(e, NULL, 0);
            if (v >= 1000 && v <= 10000000)
                cdrom_init_complete_delay = (int)v;
        }
    }
    cdrom_readtoc_complete_delay = 30000000;
    {
        const char* e = getenv("PSX_CD_READTOC_DELAY");
        if (e && e[0]) {
            long v = strtol(e, NULL, 0);
            if (v >= 100000 && v <= 60000000)
                cdrom_readtoc_complete_delay = (int)v;
        }
    }
    cdrom_pending_time_continues = 0;
    {
        const char* e = getenv("PSX_CD_PENDING_TIME_CONTINUES");
        if (e && e[0] && strtol(e, NULL, 0) != 0)
            cdrom_pending_time_continues = 1;
    }
    fm_apv2_stock_mode = 0;
    {
        const char* e = getenv("PSX_CD_APV2_STOCK");
        if (e && e[0] && strtol(e, NULL, 0) != 0)
            fm_apv2_stock_mode = 1;
    }
    fm_apv2_bypass_enabled = 0;
    {
        const char* e = getenv("PSX_FM_APV2_BYPASS");
        if (e && e[0] && strtol(e, NULL, 0) != 0)
            fm_apv2_bypass_enabled = 1;
    }
    fm_apv2_probe_active = 0;
    fm_apv2_total = fm_apv2_success = 0;
    fm_apv2_probe_play_acks = 0;
    fm_apv2_test04_count = fm_apv2_test05_count = 0;
    fm_apv2_bypass_applied = 0;
    fm_apv2_bypass_frame = 0;
    fm_apv2_bypass_seen_184 = fm_apv2_bypass_seen_188 = 0;
    fm_apv2_bypass_seen_18c = 0;
    fm_apv2_bypass_last_check_frame = UINT64_MAX;
    param_count = 0;
    response_read = 0;
    response_count = 0;
    sector_read_pos = 0;
...
void cdrom_debug_snapshot(CDROMDebugState* out) {
    ...
    out->pending_time_continues = cdrom_pending_time_continues;
    out->apv2_stock_mode = fm_apv2_stock_mode;
    out->apv2_probe_active = fm_apv2_probe_active;
    out->apv2_total = fm_apv2_total;
    out->apv2_success = fm_apv2_success;
    out->apv2_probe_play_acks = fm_apv2_probe_play_acks;
    out->apv2_test04_count = fm_apv2_test04_count;
    out->apv2_test05_count = fm_apv2_test05_count;
    out->fm_apv2_bypass_enabled = fm_apv2_bypass_enabled;
    out->fm_apv2_bypass_applied = fm_apv2_bypass_applied;
    out->fm_apv2_bypass_frame = fm_apv2_bypass_frame;
    out->fm_apv2_bypass_seen_184 = fm_apv2_bypass_seen_184;
    out->fm_apv2_bypass_seen_188 = fm_apv2_bypass_seen_188;
    out->fm_apv2_bypass_seen_18c = fm_apv2_bypass_seen_18c;
    {
        uint8_t *ram = memory_get_ram_ptr();
        if (ram) {
            out->fm_ap_state = read_le32(ram + 0x00169474u);
            out->fm_ap_timer = read_le32(ram + 0x001694C4u);
            memcpy(out->fm_ap_response, ram + 0x0016946Cu, 8u);
        }
    }
    out->i_stat = i_stat;
