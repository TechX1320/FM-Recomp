 ...
 * input bus. Reverb, noise, and IRQ timing are not modeled yet. Fixed and
 * sweep voice/main volumes are modeled at the 44.1 kHz output cadence.
 */
...
/* Volume controls live outside SpuVoice because KEYON resets decoder/ADSR
 * state but does not reset the SPU's independent L/R volume envelopes. */
static SpuSweep voice_sweep[SPU_VOICE_COUNT][2];
static SpuSweep main_sweep[2];
...
typedef struct {
    ...
    int16_t  vol_l;    /* live signed 16-bit internal volume */
    int16_t  vol_r;
    uint8_t  active;
} SpuShadowVoiceTap;
...
void spu_init(void) {
    ...
    for (int v = 0; v < SPU_VOICE_COUNT; v++) {
        spu_sweep_reset(&voice_sweep[v][0]);
        spu_sweep_reset(&voice_sweep[v][1]);
    }
    spu_sweep_reset(&main_sweep[0]);
    spu_sweep_reset(&main_sweep[1]);
#ifdef PEPSIMAN_SFX_RETRIGGER_GUARD
...
void spu_render(int16_t* out_stereo, int frames) {
    ...
    for (int f = 0; f < frames; f++) {
        int32_t mix_l = 0;
        int32_t mix_r = 0;
        const int16_t main_l = spu_sweep_read_current(&main_sweep[0]);
        const int16_t main_r = spu_sweep_read_current(&main_sweep[1]);

        if (enabled) {
            for (int v = 0; v < SPU_VOICE_COUNT; v++) {
                int16_t sample = voice_next_sample(v);
                const int16_t vol_l = spu_sweep_read_current(&voice_sweep[v][0]);
                const int16_t vol_r = spu_sweep_read_current(&voice_sweep[v][1]);
                ...
                if (sample) {
                    mix_l += ((int32_t)sample * vol_l) >> 15;
                    mix_r += ((int32_t)sample * vol_r) >> 15;
                }
            }
            {   /* T0 tap: voice sum only (pre CD mix, pre main volume). */
                int16_t vsum[2];
            ...
            mix_l = (mix_l * main_l) >> 15;
            mix_r = (mix_r * main_r) >> 15;
        }
        ...
        /* The SPU clocks voice and main volume envelopes once per 44.1 kHz
         * sample, after using the previous current-volume value for this
         * sample. This preserves the hardware's one-sample write latency and
         * lets a fixed write establish the starting level for a later sweep. */
        for (int v = 0; v < SPU_VOICE_COUNT; v++) {
            spu_sweep_clock(&voice_sweep[v][0]);
            spu_sweep_clock(&voice_sweep[v][1]);
        }
        spu_sweep_clock(&main_sweep[0]);
        spu_sweep_clock(&main_sweep[1]);
    }
     ...
     * while proven. No-op when disabled. */
    spu_shadow_process(out_stereo, frames);

    /* T1 tap: final SPU output as handed to the host layer. */
...
void spu_debug_info(SpuDebugInfo* out) {
    ...
    out->main_l = spu_sweep_read_current(&main_sweep[0]);
    out->main_r = spu_sweep_read_current(&main_sweep[1]);
    ...
    for (int i = 0; i < SPU_VOICE_COUNT; i++) {
        ...
        if (voice_sweep[i][0].control & 0x8000u) out->sweep_active_channels++;
        if (voice_sweep[i][1].control & 0x8000u) out->sweep_active_channels++;
    }
    if (main_sweep[0].control & 0x8000u) out->sweep_active_channels++;
    if (main_sweep[1].control & 0x8000u) out->sweep_active_channels++;
...
uint32_t spu_read(uint32_t addr) {
    if (addr >= 0x1F801E00u && addr <= 0x1F801E5Fu) {
        uint32_t off = addr - 0x1F801E00u;
        int voice = (int)(off >> 2);
        int lr = (int)((off >> 1) & 1u);
        if (voice >= 0 && voice < SPU_VOICE_COUNT)
            return (uint16_t)spu_sweep_read_current(&voice_sweep[voice][lr]);
    }
    if (addr >= 0x1F801C00u && addr <= 0x1F801DFFu) {
        ...
        if (idx < SPU_REG_COUNT) {
            ...
            if (addr == 0x1F801DB8u)
                return (uint16_t)spu_sweep_read_current(&main_sweep[0]);
            if (addr == 0x1F801DBAu)
                return (uint16_t)spu_sweep_read_current(&main_sweep[1]);
...
void spu_write(uint32_t addr, uint32_t value) {
    if (addr >= 0x1F801E00u && addr <= 0x1F801E5Fu) {
        ...
        int lr = (int)((off >> 1) & 1u);
        if (voice >= 0 && voice < SPU_VOICE_COUNT)
            spu_sweep_write_current(&voice_sweep[voice][lr], (int16_t)value);
        return;
    }
    if (addr >= 0x1F801C00u && addr <= 0x1F801DFFu) {
        uint32_t idx = reg_index(addr);
        if (idx < SPU_REG_COUNT) {
            ...
            if (idx < (uint32_t)SPU_VOICE_COUNT * 8u && (idx & 7u) < 2u) {
                int voice = (int)(idx >> 3);
                int lr = (int)(idx & 1u);
                spu_sweep_write_control(&voice_sweep[voice][lr],
                                        (uint16_t)value);
            }
            if (addr == 0x1F801D80u)
                spu_sweep_write_control(&main_sweep[0], (uint16_t)value);
            if (addr == 0x1F801D82u)
                spu_sweep_write_control(&main_sweep[1], (uint16_t)value);
            if (addr == 0x1F801DB8u)
                spu_sweep_write_current(&main_sweep[0], (int16_t)value);
            if (addr == 0x1F801DBAu)
                spu_sweep_write_current(&main_sweep[1], (int16_t)value);
...
void spu_get_voice_state(int idx, SpuVoiceState* out) {
    ...
    out->cur_vol_l   = spu_sweep_read_current(&voice_sweep[idx][0]);
    out->cur_vol_r   = spu_sweep_read_current(&voice_sweep[idx][1]);
...
void spu_get_global_state(SpuGlobalState* out) {
    ...
    out->main_cur_l = spu_sweep_read_current(&main_sweep[0]);
    out->main_cur_r = spu_sweep_read_current(&main_sweep[1]);
...
#define SPU_SNAP_FIELDS(X) \
    X(spu_regs) X(voices) X(voice_sweep) X(main_sweep) \
    X(transfer_addr) X(key_on_count) \
